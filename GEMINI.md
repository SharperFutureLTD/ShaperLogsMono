...
- ✅ **Phase 1 COMPLETE:** REST API Implementation (6 waves)
  ...
  - ✅ voice-to-text endpoint (OpenAI Whisper via fetch/FormData)

- 🔄 **Phase 2 COMPLETE:** Web App Integration
  - ✅ React Query infrastructure (Chunk 1) **COMPLETE**
  - ✅ Migrate ALL hooks from Supabase → REST API (Chunks 3-9) **COMPLETE**
  - ✅ Enhance 4 AI endpoints + add voice-to-text (Chunks 10-12) **COMPLETE**
  - ✅ Real-time updates (Chunk 13) **COMPLETE**

- 🔒 **Phase 3: Security Hardening (COMPLETE)**
  - ✅ **CRITICAL:** Refactor all API routes to use RLS-enabled `createUserClient` (currently using Admin Service Role)
  - ✅ Add `hono/secure-headers` (Helmet equivalent)
  - ✅ Implement Rate Limiting (especially for AI endpoints)
  - ✅ Enhance AI Redaction (Added NINO/IBAN regex)

- ⚡ **Phase 4: Efficiency & Optimization (COMPLETE)**
  - ✅ **Cost:** Switched default AI models to `gpt-4o-mini` and `claude-3-haiku` (10x-50x cheaper)
  - ✅ **Performance:** Optimized DB queries to select only necessary columns (reduced payload size)
  - ✅ **Caching:** Increased React Query `staleTime` to 60s (reduced API calls)
  - ✅ **Efficiency:** Limited AI context window to recent 20 entries (prevented token overflow)

- 🚀 **Phase 5: Scalability & Performance (COMPLETE)**
  - ✅ **Chunk 14: API Pagination.** Implement offset/cursor pagination for `work-entries` list. Update React Query hooks.
  - ✅ **Chunk 15: Redis Rate Limiting.** Replace in-memory `Map` with Redis (Upstash) for scalable rate limiting.
  - ⏸️ **Chunk 16: Async AI Processing.** Defer until user base > 1000. Current sync implementation is sufficient.

- 📜 **Phase 7: Career History & Resume Import (COMPLETE)**
  - ✅ **Chunk 17: Schema & API.** Create `career_history` table and CRUD endpoints. Add `upload-resume` parsing.
  - ✅ **Chunk 18: Frontend UI.** Create manual entry form and resume uploader components.
  - ✅ **Chunk 19: Integration.** Add to Onboarding flow and Profile page. Update AI context to include history.

- 🎯 **Phase 6: Post-Migration (NEXT)**
  - ⬜ Build Admin Backoffice (`apps/admin`)
  - ⬜ Final cleanup of legacy code (`apps/legacy-vite`)
